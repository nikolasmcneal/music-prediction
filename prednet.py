...
class PredNet(nn.Module):
	def __init__(self, input_size, R_channels, A_channels, p_max=1.0, output_mode='error',
		...
		for i in range(self.n_layers):
			...
			if self.gating_mode == 'mul':	
				...
				#batch_sz = 10 # Check this
				#in_dim = 1 # number of channels
				#x = torch.rand(size=(batch_sz, in_dim, h, w)) # why is this random?
				#print(x.shape)
				#Htm1, Ctm1 = cell.init_hidden(batch_sz)
				#print(h)
				#print(w)
				#if i >= 0:
					#ht, ct = cell.forward(x, (Htm1, Ctm1))
					#print(ct)
					#path  = "cell_states/R_l0_.pt"
	...
	def step(self, a, states):
		...
		for l in reversed(range(self.n_layers)):
			...
			#print(a.shape)
			if l == 0:
				path  = "medley_optim/cell_states/R_l{}_{}.pt".format(l, counter)
				filename, extension = os.path.splitext(path)
				#counter = 1
				while os.path.exists(path):
					counter += 1
